...
def detect_documentation_tool(docs_dir: Path, project_root: Path) -> str:
    """
    ...
    if (docs_dir / "conf.py").exists():
        return "sphinx"
    elif (project_root / "mkdocs.yml").exists():  # MkDocs config is in project root
        return "mkdocs"
    else:
...
def main():
    ...
    try:
        ...
        tool = detect_documentation_tool(docs_dir, project_root)
        ...
        if tool == "sphinx":
            ...
            generator = SphinxGenerator(config, project_root)

        else:  # mkdocs
            # Read existing mkdocs.yml to get config
            import yaml

            mkdocs_yml = project_root / "mkdocs.yml"  # MkDocs config is in project root
            ...
            generator = MkDocsGenerator(config, project_root)
