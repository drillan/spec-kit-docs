...
- **FR-038-target**: AIエージェント（Claude Code）は、README.md/QUICKSTART.md処理時、**ターゲット読者判定**を実行しなければならない（Session 2025-10-18 Q1）。README.md/QUICKSTART.mdの内容をLLMに渡し、「このドキュメントは誰をターゲットにしていますか？（一般エンドユーザー、技術者/開発者、両方）」と判定を依頼する。判定結果は統計情報としてユーザーに表示される（FR-038-stats参照）。LLM呼び出しが失敗した場合、明確なエラーメッセージを表示してプロセス全体を中断する。理由：(1) プロジェクトタイプ（アプリケーション/ライブラリ）を自動判定し、ユーザーに可視化、(2) 多言語対応（日本語・英語・その他の言語）
- **FR-038-classify**: AIエージェント（Claude Code）は、README.md/QUICKSTART.md処理時、**セクション分類**を実行しなければならない（Session 2025-10-18 Q2）。各セクション（見出し`##`, `###`）をLLMに渡し、「このセクションは誰向けですか？（エンドユーザー向け、開発者向け、両方）」と分類を依頼する。分類結果は統計情報としてユーザーに表示される（FR-038-stats参照）が、**セクション優先順位判定（FR-038-integ-b）には影響を与えない**（Session 2025-10-18 Q3, Q5）。LLM呼び出しが失敗した場合、明確なエラーメッセージを表示してプロセス全体を中断する。理由：(1) Session 2025-10-17 Q3の「LLMによるセクション優先順位判定」との一貫性、(2) 多言語対応、(3) プロジェクトごとの文脈に柔軟に対応
- **FR-038-stats**: AIエージェント（Claude Code）は、README.md/QUICKSTART.md処理時、**ターゲット読者判定（FR-038-target）とセクション分類（FR-038-classify）の結果を統計情報としてユーザーに表示**しなければならない（Session 2025-10-18 Q5）。表示形式例：「ターゲット読者: 技術者/開発者、エンドユーザー向けセクション: 3件、開発者向けセクション: 7件、両方: 2件」。この統計情報は情報提供のみを目的とし、LLM優先順位判定には影響を与えない。理由：(1) ユーザーに対してドキュメント内容の構成を可視化、(2) プロジェクトタイプとコンテンツ構成の透明性向上
